...
├── doc_templates/           # Document templates (briefings/reports) — created by `embeddables doc-templates pull` / `create`
│   └── <template-id>/
│       ├── template.mdx     # MDX content for the document template
│       └── description.txt  # Template description shown in the dashboard
└── embeddables/
...
| `embeddables doc-templates list` | List project document templates (briefings/reports). |
| `embeddables doc-templates pull` | Pull a document template to `doc_templates/<id>/`. |
| `embeddables doc-templates create` | Create a new document template interactively. |
| `embeddables doc-templates save` | Save local `doc_templates/<id>/` files back to the server. |
...
<CardGroup cols={2}>
  ...
  <Card title="FAQs" icon="circle-question" href="/faqs#cli">
