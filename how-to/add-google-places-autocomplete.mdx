...
<Steps>
  ...
  <Step title="Optional: Populate other fields with address components">
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      if (place.address_components) {
        ...
        if (streetInput) {
          ...
          // Update user data
          setUserData({ street: streetValue });
        }
    ...
    <Note>
      The `dispatchEvent(new Event('input', { bubbles: true }))` calls ensure that Embeddables recognizes the value changes in the input fields, while `setUserData()` explicitly updates the User Data for each field.
    </Note>
  </Step>
</Steps>
...
If you prefer not to add the script tag to your HTML, you can load the Google Places API programmatically through an Action. This Action should be triggered when the Embeddable loads:
