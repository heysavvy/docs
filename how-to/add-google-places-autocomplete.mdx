...
<Steps>
  ...
  <Step title="Customize the autocomplete options (optional)">
  ...
  <Step title="Populate other fields with address components (optional)">
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      if (place.address_components) {
        ...
        if (zipInput) {
          zipInput.value = components.postal_code || '';
    ...
    <Note>
      The `dispatchEvent(new Event('input', { bubbles: true }))` calls ensure that Embeddables recognizes the value changes and updates the User Data accordingly.
    </Note>
  </Step>
</Steps>
