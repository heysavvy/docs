...
Once the user clicks the generated URL, implement the following code to fetch and restore their session:
...
await fetch(
  ...
  if (data && data.entry) {
    window.Savvy.setUserData({ ...data.entry });
    window.Savvy.goToPage("flow_id", data.entry.current_page_key)
  }
})
```
