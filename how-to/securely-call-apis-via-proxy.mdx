...
<Steps>
  ...
  <Step title="Add Credentials">
    ...
    label.
    <Note>
      To rotate a secret later, use the <strong>Edit value</strong> (pencil)
      action next to a custom credential in the list. This opens a modal where
      you enter a new value; the credential keeps its unique identifier, so any
      endpoints and proxy requests that reference it keep working. A new value
      takes effect immediately, so the modal asks you to acknowledge the
      production risk before saving.
    </Note>
  </Step>
