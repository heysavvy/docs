...
<Steps>
  ...
  <Step title="Add Credentials">
    ...
    <Note>
      You can update (rotate) a credential's <strong>Value</strong> later
      without recreating it. The credential keeps the same unique identifier, so
      any endpoints or proxy requests that reference it keep working without
      reconfiguration. Internal, worker-generated credentials are read-only and
      cannot be edited.
    </Note>
  </Step>
