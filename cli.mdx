...
After upgrading, it's recommended to re-run `embeddables init` inside your project folder to refresh config files, Cursor rules, and Claude context to the latest versions bundled with the new CLI release.
...
- **`embeddables dev` now works** for Embeddables that use the CMS feature (as of v0.7.6). Important caveat: CMS content won’t load in the local preview — the Embeddable will render as if there is no `content_source` in the JSON — but CMS content will be preserved when you save.
...
<AccordionGroup>
  ...
  <Accordion title="embeddables pull doesn't show any Embeddables / no flows found">
    If you've just created a new project and `embeddables pull` returns no results or a metadata warning, try pulling by specific ID instead: `embeddables pull --id <embeddable-id>`. You can find the Embeddable ID in the Builder URL. Adding `--preserve` is also recommended on a first pull to minimize JSON noise: `embeddables pull --id <embeddable-id> --preserve`.
  </Accordion>
