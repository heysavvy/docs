  ...
  <Accordion title="Add dynamic line items">
    ...
    **Example Computed Field code:**
    ...
    function result(userData) {
      ...
      const testPriceId = 'price_1KGqjRIQ3JVMlttKzJ059opF'

      // Determine which price_id the user has selected
      const selectedProduct = userData.selected_plan === 'advanced' ? 'price_1KGqjRIQ3JVMlttKzJ059opF' : 'price_1KGqjRIQ3JVMlttKzJ059opF'

      // If we're in test mode, use the test price_id, otherwise use the selected product's price_id
      const priceId = isTestMode ? testPriceId : selectedProduct.stripe_price_id
