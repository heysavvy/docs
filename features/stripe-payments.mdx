...
<AccordionGroup>
  ...
  <Accordion title="Share a single Stripe session across multiple Stripe Checkout components">
    If you have more than one Stripe Checkout 2.0 component on the same page (for example, a Payment element and an Express Checkout element side-by-side), you can make them share a single underlying Stripe Checkout session by enabling **Use shared Stripe session** on each component.
    ...
    **How to enable it:**
    1. In the Builder, select each Stripe Checkout 2.0 component that should share the session.
    2. In the component's Options sidebar, enable the **Use shared Stripe session** toggle.
    3. Repeat for every Stripe Checkout 2.0 component on that page that should participate.
