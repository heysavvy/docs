...
- **Payment**: The standard checkout experience where customers enter their payment details (credit card, bank account, etc.) directly in the embedded form. This is the default and most common type.
- **Express Checkout**: A streamlined checkout experience that displays quick-pay buttons such as Apple Pay, Google Pay, and Link. Express Checkout lets returning customers complete purchases faster using saved payment methods in their wallets.
...
<AccordionGroup>
  ...
  <Accordion title="Prevent default click action on Express Checkout buttons">
    ...
    When **Prevent default click action** is enabled, clicking an Express Checkout button (e.g. Apple Pay or Google Pay) does **not** immediately initiate the payment flow. Instead, the click is intercepted so you can run custom logic before the payment proceeds.

    This is useful when you need to perform an action before the payment starts — for example, validating user input, updating line items, or fetching data from an external source.
    ...
    <Note>
      When this setting is enabled, you must use the [**Payment button click**](#trigger-actions-on-express-checkout-button-click) trigger to handle the intercepted click and continue the payment flow.
    </Note>
  ...
  <Accordion title="Trigger actions on Express Checkout button click">
    ...
    The **Payment button click** trigger fires when a user clicks an Express Checkout button (e.g. Apple Pay, Google Pay) and the default click behavior has been prevented via the [**Prevent default click action**](#prevent-default-click-action-on-express-checkout-buttons) setting.

    This trigger allows you to run custom logic — such as validating data, making API calls, or updating line items — before the payment flow continues.

    To use this trigger:
    1. Enable **Prevent default click action** in the Stripe component settings.
    2. Create a new Action with the logic you want to run when the Express Checkout button is clicked.
    3. Add a Trigger to the Action that fires on **Payment button click**.

    <Note>
      If **Prevent default click action** is not enabled, this trigger will not fire, and Express Checkout buttons will behave as usual.
    </Note>
