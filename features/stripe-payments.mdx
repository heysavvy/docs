...
<Steps>
  ...
  <Step title="Choose which payment methods you want to accept">
    - Under Payment methods, select the payment methods you want to accept, from the following options:
      ...
      - **US Bank Account**
      - **Buy now, pay later services**: Afterpay/Clearpay, Klarna, Affirm (note: Afterpay/Clearpay is not available in Subscription mode)
      - **Cash App**, **Amazon Pay**
    <Note>
      You need to have the relevant payment methods enabled in your Stripe account in order for them to appear for users.
    </Note>
    <Note>
      Apple Pay and Google Pay are not selectable here — they are available via the **Express Checkout** type or appear automatically based on your Stripe Dashboard settings.
    </Note>
  </Step>
...
<AccordionGroup>
  ...
  <Accordion title="Control which payment method tabs are shown">
    ...
    The payment methods currently supported in the Embeddables Stripe component are:
    - Credit/debit cards
    - Bank Account
    - Link
    - Afterpay/Clearpay (not available in Subscription mode)
    - Klarna
    - Cash App
    - Amazon Pay
    - Affirm
    ...
    Apple Pay and Google Pay are available via the **Express Checkout** type (see above), or may appear automatically depending on your Stripe Dashboard settings. They are not listed as separate options in the standard Payment checkout selector.
