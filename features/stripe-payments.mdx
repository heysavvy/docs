...
<AccordionGroup>
  ...
  <Accordion title="Share a single Stripe session across multiple Stripe Checkout components">
    If you have more than one Stripe Checkout 2.0 component on the same page (for example, a Payment element and an Express Checkout element side-by-side), you can make them share a single underlying Stripe Checkout session by setting `use_shared_stripe_session: true` on each component.
