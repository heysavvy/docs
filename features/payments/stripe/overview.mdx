...
<Steps>
  ...
  <Step title="Add a Stripe component to your Embeddable">
    ...
    - Select a Container to insert the Stripe component into, or a component that it should be added next to.
...
Optional settings are not required to accept payments, but they cover common product and UX needs.

## Pricing and line items

Dynamic prices, CMS-managed prices, and discounts.
...
<AccordionGroup>
  <Accordion title="Add dynamic line items">
    ...
    2. Reference the key of that Computed Field in the Line Items section of the Stripe component.
...
## Customer data and Stripe session

Shipping, templating, customer creation, IDs, metadata, and shared sessions.
...
<AccordionGroup>
  ...
  <Accordion title="Store the selected plan/product and payment status in User Data">
    ...
    [stored in the 'Embeddables Cloud Context'](/features/user-data#where-user-data-is-stored) - i.e. stored in Embeddables' database.
...
## Checkout UI and payment methods

Button copy, validation, layout, and which methods appear.
...
## Holds and default payment methods

Authorization holds and default payment method behavior.
...
## Events and Express Checkout

Triggers for payment lifecycle and Express Checkout click handling.
...
<AccordionGroup>
  ...
  <Accordion title="I'm getting an error about a missing restricted key">
    This means that you need to add a Stripe Restricted Key with the [scopes/permissions listed above](#keys-and-ids-you-need) to your Embeddables project.
    ...
    **How to fix:**
    ...
    - Make sure the key has the correct permissions, listed [above](#keys-and-ids-you-need).
    - Click **Create** to save the credential.
...
<CardGroup cols={2}>
  <Card title="CMS" icon="table" href="/features/cms">
    Manage Stripe prices outside the Builder.
  </Card>
  <Card title="Actions" icon="code" href="/features/actions">
    Run code on payment success, failure, and other triggers.
  </Card>
</CardGroup>
