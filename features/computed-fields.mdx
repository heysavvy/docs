...
- **Save to Cloud Only**: The result is saved to the cloud database but not stored locally in cookies/localStorage
...
When enabled, the computed field result is cached and won't recalculate if the inputs haven't changed. This is useful for expensive computations.

<Note>
Caching is done by comparing the input values. If the inputs are the same as the last computation, the cached result is returned without re-running the function.
</Note>
