...
## Translating with the AI chat

The Builder's AI chat can handle translations for you. Simply ask it in natural language — for example:

> "Translate this page to Spanish"

The AI will:
1. Register the language if it is not already set up (e.g. Spanish / `es`).
2. Translate all supported text fields on the current page.
3. Switch the Builder preview to the new language so you can review the result immediately.

You can also ask for partial translations, translate a specific component, or add multiple languages one by one. The AI uses the same language keys and translation storage as the manual workflow described below.
