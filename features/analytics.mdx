...
The **Analytics Hub** is a dedicated space in the Embeddables Web App for exploring how your Embeddables perform. Open it from the **Analytics** tab in the project sidebar.

<Note>
The Analytics Hub is rolled out on a per-project basis. If you don't see the **Analytics** tab, an admin can enable it in your project's settings.
</Note>

The Hub has two main areas:

- **Home** — Lists your Embeddables along with any saved views, and a **New view** button to start a report.
- **Report** — The report builder, where you explore data for a single Embeddable over a date range.

To build a report:

<Steps>
  <Step title="Pick an Embeddable and date range">
    Use the context bar at the top of the report to select the Embeddable you want to analyze and the date range to report on.
  </Step>
  <Step title="Configure the report">
    In the controls panel, set the **Measure**, **Rows**, and **Columns**, apply **Filters**, and use the **Exclude test users** toggle to leave test traffic out of your numbers.
  </Step>
  <Step title="Read the tables">
    Reports render **funnel**, **breakdown** (for example by UTM source), and **conversions** tables, all wired to live data.
  </Step>
  <Step title="Save the view">
    Save a report as a view so you can return to the same configuration later. Saved views appear on the Analytics Home.
  </Step>
</Steps>
