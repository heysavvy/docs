...
- Tags are reusable across your Embeddable
...
Each component should have one main ("primary") tag that defines its default appearance and behavior (e.g., `standard_button` or `pricing_card`). The primary tag includes all base styles for a component, except styles from secondary tags or single-style overrides.

**Secondary tags**

Secondary tags build on primary tags to create specific variations without repeating styles. They can overwrite primary tag styles, but shouldn't replace the majority of them.
...
For single-style overrides (like margin or padding), use Tailwind-style tags in snake_case (e.g., `mt_10`, `mb_0`, `pt_8`) instead of creating new secondary tags. Include spacing styles in the primary tag only if they're universally useful across all instances. For layout spacing between components, consider using the `gap` property in the parent container instead.
