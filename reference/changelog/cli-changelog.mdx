...
- **`embeddables doc-templates` command group** — New `doc-templates` namespace for managing project document templates (briefings/reports) from the CLI:
  - `embeddables doc-templates list` — Print a table of the project's document templates (ID, status, title, last updated).
  - `embeddables doc-templates pull -i <id>` — Fetch a template and write `template.mdx` and `description.txt` to `doc_templates/<id>/` (use `-o, --out <dir>` to override the base directory).
  - `embeddables doc-templates create` — Interactively prompt for title, status, and optional `cron_utc`, create the template, and write starter files to `doc_templates/<id>/`.
  - `embeddables doc-templates save -i <id>` — Upload the local `template.mdx` and `description.txt` back to the server.

  Each command resolves the project from `-p, --project-id`, `embeddables.json`, or an interactive prompt, and requires `embeddables login`.
- **`docs/document-templates.md` written by `init` / `upgrade`** — `embeddables init` and `embeddables upgrade` now also write `docs/document-templates.md`, a dedicated MDX guide for AI assistants when editing or creating document templates. The shared short-pointer rule files now reference this guide alongside the main `AI-README.md`.
...
- **AI prompt rules: doc templates pointer** — The shared short-rule body now references `docs/document-templates.md` so AI assistants pick up the doc-template guide before editing or creating templates.
