...
- 🚫 **Stripe Checkout: Disable Button Until Page Is Valid** – A new **Disable button until page is valid** toggle in the Stripe Checkout 2 options keeps the checkout button disabled until all required fields on the page pass validation. The checkout button in the Designer also gains a `disabled` state so you can style it distinctly when locked.
- 🧭 **`goToNextPage`, `goToPrevPage`, and `triggerAction` on `window.Embeddables`** – Three methods are now available on the `window.Embeddables` object for use from your host page: navigate forward (`goToNextPage`) or backward (`goToPrevPage`) through the flow, or fire a named action by its ID (`triggerAction`).
