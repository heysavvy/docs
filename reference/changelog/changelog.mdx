...
- 📊 **Funnel Percentage Base Toggle** – The Analytics funnel table now has a **Funnel base** control (shown when **Rows** is set to **Funnel Step**) that chooses what each step's percentage is measured against: **Total traffic** — all unique visitors in the date range (the default) — or **First step** — visitors who reached the funnel's first visible step. In First step mode, visitors who never reached step 1 are excluded from the counts (not just the percentages), so the first visible step always reads 100%. The percentage column header updates to match (**% of total traffic** / **% of first step**) and carries a tooltip explaining the active denominator and how it differs from the embeddable Funnel tab. Your chosen base is saved with each view.
- 🧪 **Exclude Test Users by Field** – The Analytics **Exclude test users** control now lets you choose which of an embeddable's input fields to check from a searchable dropdown, instead of typing field keys by hand. Any contact whose latest value for a selected field contains "test" is filtered out of every metric — funnel, breakdown, conversions, and total traffic.
</Update>
