...
- 🎬 **Disable Interactions During Transitions** – A new **Disable Interactions During Transitions** toggle is available in the Builder's *Entire Embeddable* settings (select nothing / press ESC, then open the **Options tab**). When enabled, Option Selector and Custom Button clicks are blocked while a page transition is animating, so users can't accidentally double-advance or skip a page mid-animation. Inputs, file uploads, and other components are unaffected. Newly created Embeddables have this enabled by default.

### 🐞 Bug Fixes

- **Publish Modal Scrolls on Small Screens** – The Save & Publish panel's lower section — the Staging and Production environment cards and the **Push to selected environment** button — is now wrapped in a scrollable container, so the publish action stays reachable on short viewports instead of being cut off and blocking publishing.
</Update>
