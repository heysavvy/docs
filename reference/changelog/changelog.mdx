...
- 🚀 **Engine v4 Upgrade**:
  - 🧼 Removed all hidden internal styles – Fully redesigned style generation with strict breakpoint ordering and clean component templates.
...
- 📊 **Step Drop-off Metrics** – Displays drop-off percentages between steps and the % of total contacts to help identify friction points.
...
- 👥 **User Management Upgrade** – Admins can now remove members from projects directly in the users table, with built-in safeguards.
- 🧠 **Real-time Schema Enforcement** – Implemented Zod across the Builder to validate JSON schemas instantly, with clear warnings for any issues as you work.
...
- 🪄 **Split test individual components:** You can now experiment withhiding/showing components within a test. Unsure if the social proof container increases click throughs? Now you can test just showing/hiding it without duplicating the entire page and running an A/B split on the page.
...
- Button tooltips**: give your users more info in case they need it, with easy-to-add tooltips on butto
...
- Default User Data** setting available in Whole Embeddable optio
### ⚡️ Improvements
