...
- 💡 Helpful Data Tooltips – Added tooltips for "Health Data", "Personally Identifiable Information", and "Contact Info" in the Data Schema table. No more guessing!
- 📝 Conversion Dropdown – Creating a conversion now uses a dropdown of available options instead of a text box. No more typos or broken analytics!
...
- 🪄 **Split test individual components:** You can now experiment with hiding/showing components within a test. Unsure if the social proof container increases click through? Now you can test just showing/hiding it without duplicating the entire page and running an A/B split on the page.
