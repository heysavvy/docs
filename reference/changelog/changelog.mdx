...
- 🌐 **Stripe Checkout 2 Translation Support** – Pay button text and success message in Stripe Checkout 2 components now respond to language changes when translation variants are configured, keeping your payment UI fully localised.
- 📡 **`trackCustomEvent` on `window.Embeddables`** – `trackCustomEvent` is now exposed on `window.Embeddables` as well as `window.Savvy`, keeping the two global APIs consistent.

### ⚡️ Improvements

- **`trackCustomEvent` returns a Promise** – `window.Savvy.trackCustomEvent()` and `window.Embeddables.trackCustomEvent()` now return a `Promise<void>`, so you can `await` the call before redirecting or performing follow-up actions.
</Update>
