...
- 🧪 **Create Experiment API** – New Bearer-authenticated endpoint (`POST /api/embeddables/create-experiment`) to create experiments programmatically, with automatic variant weighting distribution.
- 🚀 **Version Publish API** – New Bearer-authenticated endpoints (`POST /api/embeddables/set-version-status` and `POST /api/embeddables/publish`) to promote versions to staging or production programmatically, matching the Builder's promotion rules.

### ⚡️ Improvements

- Version promotion via API is now idempotent — promoting a version that is already at the target status no longer returns an error.
- Promoting a version to production now requires it to be on staging first, consistent with the Builder behavior.
