...
- 📊 **Traffic & Audience Breakdowns (Crosstabs)** – The Analytics Hub now supports two-dimensional breakdowns: set both a **Rows** and a **Columns** dimension (for example Medium × Traffic Source) to build a crosstab report powered by new ClickHouse pivot queries. The Rows and Columns pickers now disable only the conflicting dimension instead of blocking a whole group, and unsupported combinations are hidden from the dropdowns entirely.
- 👁️ **Show / Hide Rows & Columns in Analytics Tables** – Funnel, conversions, and breakdown tables share a redesigned show/hide experience: a toolbar above each table toggles editing, individual rows and columns can be hidden and restored, hidden columns are managed from a dropdown with a count badge, and tables scroll horizontally with a frozen first column so metric columns stay aligned. Hidden selections persist with the saved view.
- 🧪 **Exclude Test Users** – A new **Exclude test users** switch and field-key multiselect in the Analytics Hub lets you filter out test contacts from reports by matching selected answer keys. Matching is substring-based on `test`, so values such as `contest@` may also be excluded.
- 🤖 **Assign AI-Created Tasks to Any Project Member** – The `create_task` and `update_task` MCP tools now accept an `assignee` by name or email and resolve it to any member of the project, not just the calling user. A companion lookup tool surfaces project members (excluding the AI assistant) so assignments can be resolved automatically.

### ⚡️ Improvements

- **Collapsible Report Settings Panel** – The Analytics Hub report controls now live in a collapsible right sidebar with a **Settings** header and toggle. The collapsed state is remembered per project, and the panel stays mounted while collapsed so report validation keeps running.
- **Collapsible Hidden-Rows Divider** – Hidden rows and steps now stay in place behind a compact "N rows/steps hidden" divider instead of sinking to the bottom or distorting the table. Expand it to restore items individually or use **Show all**.
- **Clearer Table Editing Controls** – The table editing toggle is now labelled **Show/hide rows and columns**, and columns are hidden with an eye-off icon instead of a close (X) icon.
- **Consistent Table Typography** – The total-traffic row in the conversions table now uses the same font size as the other rows, with emphasis conveyed through weight and row tint instead.
- Removed a redundant note under the funnel percentage-base control that duplicated the helper text already shown above it.

### 🐞 Bug Fixes

- **Analytics Date Picker Across Timezones** – The Analytics Hub date picker no longer shows future dates or a default range that lands on the wrong day when the browser and project timezones differ; presets and the maximum selectable date are now region-aware and align with the project's "today".
- **Date Range Sticking to the Wrong Timezone** – Auto preset ranges are now re-derived once the project region resolves after load, without overwriting an explicit date range the user has already picked.
- **Hidden Rows & Columns Persistence** – Saved hidden rows and columns no longer reappear after a reload. A transient empty embeddable selection during navigation no longer wipes just-hydrated hidden rows, and stale hidden keys are cleared correctly when the embeddable, sub-key, or column dimension changes.
- **Breakdown Column Truncation in Production** – Long breakdown column headers now truncate with an ellipsis (and reveal the full value on hover) in production builds, where competing CSS width rules previously prevented the cap from applying.
</Update>
...
- **Experiment Date/Time Filter Crash** – Clearing a time segment with Backspace or Delete in the date/time range filter on the experiment page no longer crashes the page. The time-change handlers now ignore the transient `null` value React Aria reports while a segment is mid-edit, instead of attempting to read `.hour` / `.minute` from it.
</Update>
...
- 🔐 **Internal Credential Lookup RPC** – The Custom Credentials worker now exposes a `getInternalCredential` RPC so other Embeddables Workers can fetch internal credentials by project, key type, and environment. Live credentials are matched and the most recently created one is returned; when no live credential matches the call returns `null`, letting callers cleanly distinguish "not configured" from hard errors.

### 🐞 Bug Fixes

- Copy/pasting nested global components in the Builder no longer flattens their hierarchy — child components inside copied containers now keep their parent/child structure instead of becoming siblings.
- Merge replay during concurrent Builder edits now anchors new and moved components only to same-bucket peers (same parent, same `_location` slot, or same root group), preventing accidental hierarchy or slot corruption when other people are editing the same embeddable.
</Update>
...
- 📊 **Payload Metric Key for Experiments** – Experiments can now track a custom payload metric in addition to conversions. In the Edit Experiment modal, a new **Event metric key** dropdown appears once you have selected impact metrics whose conversions have payload metric keys configured. Select one key to enable per-variant metric aggregation in experiment results.

### 🐞 Bug Fixes

- **Team Invite Modal Auto-Close on Pending Invite** – Re-inviting a user who already has a pending invitation now automatically dismisses the invite modal after the error toast appears, instead of leaving the dialog open in an ambiguous state.
...
- 🔐 **Credential Templating in Proxy Request Bodies** – The Secure Proxy now supports substituting `{{ credential_id }}` placeholders in request bodies. Send `X-Use-Body-Templating: true` on any POST, PUT, PATCH, or DELETE request and the proxy will replace credential placeholders in the body before forwarding, using the same allowlisted credentials as URL and header substitution. The header is stripped from the outbound request. Omit the header (or leave it unset) to keep the default streaming pass-through, which avoids buffering and is recommended for large or binary payloads.
</Update>
...
<Update label="31 March 2025">
  ...
  you can do in the Builder. - [Watch it here](/builder/overview) and be fully
