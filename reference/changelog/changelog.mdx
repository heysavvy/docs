...
- 🛠️ **JSON Error Fix Button** - Added a button in the Builder layers section to automatically fix all detected JSON errors.
...
- Updated the warning text for scenarios where `parent_id` matches `parent_key`.
...
- Fixed PBB (Probability to Be Best) calculation for experiments with low traffic variants to ensure accurate results.
