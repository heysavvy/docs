...
- 🧪 **Conversion Filter Dropdown** – The experiments page now features a dropdown to select which conversion is used for table filtering when several are available.
- 📁 **FileUploader v2: Multi-File & Validation** – The FileUploader component now supports uploading multiple files and custom validation error messages. Upgrade to version 2 for these features.
- 🗂️ **OptionSelector Key Warnings** – The layers tab now warns you if any buttons inside an OptionSelector are missing a key, helping prevent configuration issues.
- 💸 **Stripe Checkout Discounts** – You can now add a discount to Stripe Checkout via a single promo code or coupon (not both at once).
- ⏹️ **AI Chat Stop Button** – Added a "stop" button to AI chat, letting you halt message generation early.
...
- Submitting 10k entries or other large files now works as expected.
...
- Experiments are no longer tracked while they are paused.
</Update>
