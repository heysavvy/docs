...
- 🗓️ **Multi-Select Document Template Scheduling** – Document template schedules now use a day & hour grid instead of a single Daily/Weekly frequency dropdown. Pick any combination of weekdays and hours — for example "Monday, Wednesday, Friday at 09:00 and 17:00" — and switch the template to **Active** to start the schedule. Day and Time only become required once a template is active, so drafts can be saved with an empty schedule.
- 🌐 **Browser Filter on Users List** – The Users page now has a **Browser** filter to narrow down contacts by browser (Chrome, Firefox, Safari, etc.). The filter is multi-select, syncs with the URL so filtered views are shareable, and uses the same UAParser-based browser names as the Device column.

### ⚡️ Improvements

- **Smarter Frequency Badges** – The frequency badge on document templates now summarises multi-day, multi-hour schedules as **Runs N× per week**, with a tooltip listing each selected day and time. Single daily and single weekly schedules continue to show **Runs once per day** / **Runs once per week**.
- **Briefings Track Their Source Template** – Briefings created from a document template now record the originating template ID, so future analytics and exports can trace each briefing back to the template that produced it.
</Update>
