...
<Update label="10 July 2026">
### 🔥 Features and Updates

- 📊 **Hide & Restore Analytics Table Rows and Columns** – The Analytics funnel, conversions, and breakdown tables now share an **Edit rows and columns** toolbar above the table. Turn it on to hide individual breakdown rows and columns inline, then use the **Hidden columns** dropdown (with a count badge) to bring them back. The first column stays frozen while the rest of the table scrolls horizontally, so wide breakdowns remain readable — and your hidden-column selections are saved with the view.
- 🤖 **Assign Tasks to Any Project Member via MCP** – The `create_task` and `update_task` MCP tools can now assign tasks to other project members instead of only the caller. Provide an `assignee` name or email — resolved server-side with case-insensitive matching — or an `assignee_id` UUID (supplying both is rejected). A new **`get_project_members`** MCP tool lists a project's assignable members so the assistant can look up the right person before assigning.
</Update>
