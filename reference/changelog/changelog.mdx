...
- 🌐 **Full Navigation for All Customers** – Goals, Tasks, QA, Inbox, and the full sidebar navigation (including "Improve" and "Content" section headings) are now available to all customers. The "+" Create dropdown also now includes New Task, New Goal, and New QA Test for everyone — no special access required.
...
- The analytics page is now consistently named **Insights** across the board (previously shown as "Overview" to some customers).
- Slack integration settings in Project Settings → General are now accessible to all customers.
- Experiment status summaries (win-rate metrics on the experiment detail page) are now visible to all customers.
- Submissions CSV downloads are significantly faster — chunks are now fetched in parallel rather than sequentially.
