...
- 🔀 **Reorder Saved Views in the Analytics Hub** – Saved views within an embeddable group on the Analytics Hub Home can now be manually reordered by drag-and-drop. Grab the new drag handle on any saved view row and drop it into place; the order is saved and shared across your project, so it stays intentional instead of always reflecting the last-edited view. Groups you haven't manually ordered continue to sort by newest first. Reordering supports mouse, touch, and keyboard, and updates optimistically with automatic rollback if the save fails.

### ⚡️ Improvements

- **Streamlined Conversion Rate Setup** – The benchmark comparison has been removed from conversion rates. The **Benchmark** dropdown no longer appears in the create/edit conversion rate modal, and the **Benchmark** column has been removed from the conversion rates settings table, leaving a simpler flow focused on the base and conversion metrics.
</Update>
