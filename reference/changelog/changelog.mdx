...
- 🔑 **Edit Custom Credential Values** – Custom credentials in the **Credentials & Endpoints** settings can now have their value updated (rotated) in place, instead of only being created or deleted. The credential's unique identifier is preserved, so any endpoints or Secure Proxy rules that reference it keep working without reconfiguration. Secret values are re-encrypted and re-redacted on update, while internal, worker-generated credentials remain read-only and cannot be edited.
</Update>
